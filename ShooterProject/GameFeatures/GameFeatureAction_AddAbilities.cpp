...
#include "ShooterProject/AbilitySystem/ShooterProjectAbilitySystemComponent.h"
#include "ShooterProject/AbilitySystem/ShooterProjectAbilityAttributeSet.h"
#include "ShooterProject/AbilitySystem/AbilityInputBindingComponent.h"

#define LOCTEXT_NAMESPACE "ShooterProjectFeatures"
...
void UGameFeatureAction_AddAbilities::AddAdditionalAssetBundleData(FAssetBundleData& AssetBundleData)
{
	if (UAssetManager::IsValid())
	{
		...
		{
			for (const FShooterProjectAbilityMapping& Ability : Entry.GrantedAbilities)
			{
			...
			for (const FShooterProjectAttributesMapping& Attributes : Entry.GrantedAttributes)
			{
...
{
	UE_LOG(LogShooterProject, Error, TEXT("Added ability"), *Actor->GetPathName());
	if (UAbilitySystemComponent* AbilitySystemComponent = FindOrAddComponentForActor<UAbilitySystemComponent>(Actor, AbilitiesEntry))
	{
		...
		for (const FShooterProjectAbilityMapping& Ability : AbilitiesEntry.GrantedAbilities)
		{
			if (!Ability.AbilityType.IsNull())
			{
				...
				if (!Ability.InputAction.IsNull())
				{
					...
					{
						 UE_LOG(LogShooterProject, Error, TEXT("Failed to find/add an ability input binding component to '%s' -- are you sure it's a pawn class?"), *Actor->GetPathName());
					}
				}
		...
		for (const FShooterProjectAttributesMapping& Attributes : AbilitiesEntry.GrantedAttributes)
		{
			if (!Attributes.AttributeSetType.IsNull())
			{
				TSubclassOf<UShooterProjectAbilityAttributeSet> SetType = Attributes.AttributeSetType.LoadSynchronous();
				if (SetType)
				{
	...
	{
		UE_LOG(LogShooterProject, Error, TEXT("Failed to find/add an ability component to '%s'. Abilities will not be granted."), *Actor->GetPathName());
	}
}
